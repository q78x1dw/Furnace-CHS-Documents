...
用户界面包括几个组成部分,这其中有一些需要解释一下the user interface consists of several kinds of components, some of which benefit from explanation.
...
窗口停靠后,标题栏就会变为选项卡栏,然后左上角的"收起"三角形的功能就会改变.when a window is docked, its title bar turns into a tab bar, and the function provided by the "collapse" triangle at the top left changes.

![tab2](tab2.png)

如果点击这个三角,会出现一个列有选项卡的菜单,或者如果只有一个选项卡,就会有一个单独的选项:"隐藏选项卡栏".if this triangle is clicked, a menu will appear with a list of tabs, or a single option if there's only one tab: "Hide tab bar".
selecting this option will hide the tab bar of that window.

![tab3](tab3.png)

要想把它带回来,就点击左上角to bring it back, click on the top left corner.

要想解除一个窗口的停靠状态,就把他的选项卡脱离它停靠的位置,然后他就会重新浮动了.to undock a window, drag its tab away from where it is docked. then it will be floating again.
