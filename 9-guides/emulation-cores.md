# 选择模拟器核心choosing emulation cores

Furnace通过使用**模拟器核心**,来尽可能地获得电子游戏的真实声音.在某些情况下有多个核心可选,每个都有自己的优点与缺点.他们的区别如下.Furnace achieves the authentic sound of videogame hardware by emulating sound chips as accurately as possible, using **emulator cores**. in some cases there are multiple cores to choose from, each with different strengths and weaknesses. here are the major differences between them all.

- **YM2151 core**:
  - **ymfm**: 默认的播放核心.相比Nuked-OPM使用的CPU少得多,但是更不精确.推荐用于移动设备,或者旧的硬件.default playback core. much less CPU usage than Nuked-OPM, but less accurate. recommended for users with mobile, last-gen or earlier hardware.
  - **Nuked-OPM**: 默认的渲染核心.比ymfm精确的多,因为模拟器是参考了真正的YM2151的dieshot.对CPU压力很大,推荐有新硬件的用户使用.default render core. much more accurate than ymfm, due to the emulator being based on an image of the die map taken from a real YM2151. very CPU heavy, only recommended for users with recent hardware.

- **YM2612 core**:
  - **Nuked-OPN2**: 默认核心.比Nuked-OPM对于CPU压力更小.可以模拟YM2612的任何变体.default core. lighter on the CPU than Nuked-OPM, can be used to simulate any variant of YM2612.
  - **ymfm**: 同上ymfm. same as ymfm above.
...
- **OPN/OPNA/OPNB cores**:
  - **ymfm only**: CPU使用更小,FMlower CPU usage, less accurate FM.
