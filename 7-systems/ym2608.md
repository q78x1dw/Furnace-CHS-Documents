...
类似YM2203 但是有两倍FM通道,立体声, 一个ADPCM通道,和内置的鼓点音色("节奏")! like YM2203, but with twice the FM channels, stereo, an ADPCM channel and built-in drums ("rhythm")!

这是NEC PC-88VA和之后型号的PC-98系列电脑可用的音源之一it was one of the available sound chips for the NEC PC-88VA and later models of PC-98 series of computers.

YM2610(OPNB)和YM2610B芯片类似这个,但是内置的鼓替换为6个采样通道. the YM2610 (OPNB) and YM2610B chips are very similar to this one, but the built-in drums have been replaced with 6 sample channels.

## 效果effects

- `10xy`: **设置LFO参数set LFO parameters.**
  - `x` 开关LFOtoggles the LFO.
  - `y` 设置速度sets its speed.
...
这个芯片使用this chip uses the [FM (OPN)](../4-instrument/fm-opn.md), [AY-3-8910/SSG](../4-instrument/ay8910.md), [ADPCM-A](../4-instrument/adpcm-a.md) 和and [ADPCM-B](../4-instrument/adpcm-b.md) 乐器编辑器instrument editors.
...
CSM, 或者or "复合Composite 正弦波Sine 模式Mode", 采用了一个定时器设置为"CSM 定时器" 通道的频率. 在定时器每次触发, 他都生成按键开和按键关命令来重置通道3上面的所有运算器的相位, 然后迫使他们的包络重置为在释放点重新开始. involves a timer matching the frequency of the note in the "CSM Timer" channel. each time it triggers, it generates key-on and key-off commands to reset the phase of all operators on channel 3 and force their envelopes to restart at the release point. 这可以用于创造语音共振峰(语音合成!) 或者其他的复杂效果. 独立于这个芯片的实现, 这个技巧叫做"oscillator sync". this can be used to create vocal formants (speech synthesis!) or other complex effects. outside this chip's specific implementation, the technique is known as "oscillator sync".
